/**
 * services-card.js  —  v2
 * Unraid Docker service on/off grid.
 * Extracted from technology-card. Standalone — use independently anywhere.
 *
 * ── INSTALLATION ──────────────────────────────────────────────────────────────
 * 1. Copy to /config/www/cards/services-card/services-card.js
 * 2. HA → Settings → Dashboards → Resources → Add:
 *      URL:  /local/cards/services-card/services-card.js
 *      Type: JavaScript Module
 *
 * ── CONFIG ────────────────────────────────────────────────────────────────────
 * type: custom:services-card
 * entities:                                          # switch or binary_sensor entities
...
class ServicesCardCard extends HTMLElement {
  constructor() {
    super();
  ...
  static getConfigForm() {
    return {
      ...
      { name: 'jellyfin', label: 'Jellyfin (switch)', selector: { entity: { domain: ['switch','binary_sensor'] } } },
      { name: 'sonarr',   label: 'Sonarr (switch)',   selector: { entity: { domain: ['switch','binary_sensor'] } } },
      { name: 'radarr',   label: 'Radarr (switch)',   selector: { entity: { domain: ['switch','binary_sensor'] } } },
      { name: 'seerr',    label: 'Seerr (switch)',    selector: { entity: { domain: ['switch','binary_sensor'] } } },
    ] },
      assertCustomElement: 'services-card',
    };
  }
  ...
  // ── Render / Patch ──────────────────────────────────────────────────────────
  _render() {
    const inner = this._buildServices();
...
customElements.define('services-card', ServicesCardCard);
