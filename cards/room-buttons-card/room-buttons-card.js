  ...
  _openPopup(btn) {
    ...
    popup.querySelectorAll('.fan-tile[data-fan-tile]').forEach((tileEl, fi) => {
      ...
      tileEl.addEventListener('click', e => {
        if (e.target.closest('.spd-row')) return;
        const current = this._fanCurrentStep(cfg);
        const next    = (current + 1) % (speeds + 1);
        this._setFanSpeed(cfg, next);
        setTimeout(() => { if (this._popupOpen) this._openPopup(btn); }, 350);
      });

      // Speed segment buttons
      tileEl.querySelectorAll('.spd-btn[data-fan-step]').forEach(el => {
        el.addEventListener('click', e => {
          e.stopPropagation();
          const step = parseInt(el.dataset.fanStep);
    ...
    // Brightness sliders
    toggles.forEach((cfg, i) => {
      if (!cfg.entity.startsWith('light.') || !this._isToggleOn(cfg.entity)) return;
      const track = popup.getElementById(`brt-${i}`);
      if (!track) return;

      const update = clientX => {
        const rect = track.getBoundingClientRect();
        const pct  = Math.max(1, Math.min(100, Math.round((clientX - rect.left) / rect.width * 100)));
        const fill  = popup.getElementById(`brf-${i}`);
        const thumb = popup.getElementById(`brth-${i}`);
        const val   = popup.getElementById(`brv-${i}`);
      ...
      let lastPct = this._brightness(cfg.entity) || 0;

      const onMouseMove = e => { lastPct = update(e.clientX); };
      const onTouchMove = e => { e.preventDefault(); lastPct = update(e.touches[0].clientX); };
      ...
      const commit = () => {
        ...
        this._setBrightness(cfg.entity, lastPct);
      };

      track.addEventListener('mousedown', e => {
        e.preventDefault();
        lastPct = update(e.clientX);
      ...
      track.addEventListener('touchstart', e => {
        e.preventDefault();
        lastPct = update(e.touches[0].clientX);
