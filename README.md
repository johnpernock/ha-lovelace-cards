...
| Card | Folder | Version |
|------|--------|---------|
| Room Controls | `cards/room-controls-card/` | v63 |
| Room Buttons | `cards/room-buttons-card/` | v4 |
| Camera Layout | `cards/camera-layout-card/` | v4 |
...
| Date | Summary |
|------|---------|
| Mar 2026 | **Room card polish + responsive camera.** `room-controls-card` v60–v63: thermostat grey backgrounds removed (transparent); +/− buttons transparent; slider thumb min position 4% (no edge overlap); thermostat block auto-hides when entity missing from HA; all lights slider no longer dims when off; yard area buttons dim at 50% opacity when off. `camera-layout-card` v4: responsive stacking breakpoint raised 480px → 700px. |
